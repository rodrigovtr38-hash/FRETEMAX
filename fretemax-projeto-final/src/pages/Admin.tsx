...
// CTO-Log: Remoção TOTAL de React Context (Crash Resolvido).
// Status: Retenção de Estado local para garantir a inserção da Chave PIX antes do Unmount.
// =========================================================
...
export default function DriverActiveTrip({ freteId }: DriverActiveTripProps) {
  ...
  const handleFinalizeWithPix = async () => {
    ...
    try {
      ...
      // O Motorista.tsx detectará essa mudança em tempo real e fechará esta tela automaticamente.
    ...
    try {
      if (frete.status === 'coletando') {
        ...
        if (isMultiDrop && paradaAtualIndex + 1 < pinEntregas.length) {
           ...
           // 🔥 CTO FIX: PIN VALIDADO! Apenas abre a tela do PIX localmente.
           // Não chame o backend para finalizar a viagem ainda, senão a tela fecha.
