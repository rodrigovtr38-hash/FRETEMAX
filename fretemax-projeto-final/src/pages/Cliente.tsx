    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-10">
      <nav className="bg-slate-950 p-4 flex items-center justify-between shadow-xl">
        <div className="flex items-center gap-2">
          {step === 'busca' && <ArrowLeft onClick={handleBack} className="text-white cursor-pointer w-6 h-6 mr-2" />}
        ...
        {step === 'busca' ? (
             ...
             {orderData?.status === 'motorista_a_caminho' ? (
               <div>
                 ...
                 <div className="mt-6 p-6 bg-slate-900 rounded-3xl text-white">
                    <p className="text-[10px] font-bold text-blue-400 uppercase mb-1">Motorista</p>
                    <p className="text-xl font-black mb-4">{orderData.motoristaNome}</p>
                    <button onClick={() => window.open(`https://wa.me/55${orderData.motoristaZap.replace(/\D/g,'')}?text=Olá, sou seu cliente FRETOGO!`)} className="w-full bg-green-500 py-4 rounded-xl font-black uppercase">FALAR NO WHATSAPP</button>
                 </div>
               </div>
             ) : (
               <div className="py-8">
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 p-5 rounded-3xl text-white border-l-8 border-yellow-400 shadow-lg">
               ...
               <p className="text-base font-bold italic">Preencha os dados abaixo.</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
               <input className="p-4 bg-white rounded-2xl font-bold border border-slate-200" placeholder="Peso (kg)" onChange={e => setCarga({...carga, peso: e.target.value})} />
               <input className="p-4 bg-white rounded-2xl font-bold border border-slate-200" placeholder="Material" onChange={e => setCarga({...carga, tipo: e.target.value})} />
            </div>
            ...
            <div className="bg-slate-950 p-8 rounded-[2.5rem] text-center shadow-2xl border-b-8 border-blue-600">
               <p className="text-[11px] font-black uppercase text-blue-400 mb-1 tracking-widest">Valor do Frete</p>
