...
export default function Cliente() {
  ...
  // 5. PWA (CLIENTE) - Suporte para instalação nativa
  useEffect(() => {
  ...
  // 4. OTIMIZAÇÃO DE LEITURA (PERFORMANCE): Escutando apenas o documento específico
  useEffect(() => {
    if (currentOrderId) {
  ...
  const handleContratar = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'fretes'), {
        ...
        // 3. VALORES NUMÉRICOS (Numbers para Backend Correto)
        ...
        valorFormatado: valorFormatado, // Texto para UI
        
        // 2. PADRONIZAÇÃO DE CAMPOS (Compatibilidade com Motorista.tsx)
        cidadeOrigem: coleta.bairro,
        origemRua: `${coleta.rua}, ${coleta.num}`,
        cidadeDestino: entrega.bairro,
        destinoRua: `${entrega.rua}, ${entrega.num}`,
        peso: carga.peso,
        material: carga.tipo, // tipoCarga -> material
        
        // 1. CORREÇÃO DE STATUS (Fluxo Financeiro Uber)
        status: 'aguardando_pagamento',
        pagamentoStatus: 'pendente', // Atualizado via webhook posteriormente
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-10">
      {/* PWA CTA Bar */}
      {deferredPrompt && (
        <div className="bg-blue-600 p-3 flex items-center justify-between px-6 sticky top-0 z-[60] shadow-lg animate-in fade-in">
          <div className="flex items-center gap-2 text-white">
            <Download className="w-4 h-4" />
            <span className="text-[10px] font-bold uppercase tracking-wider">Instalar FreteGO no Celular</span>
          </div>
          <button onClick={handleInstallPWA} className="bg-white text-blue-600 text-[10px] font-black px-4 py-1 rounded-full uppercase">Baixar</button>
        </div>
      )}
      ...
      <div className="max-w-md mx-auto px-4 mt-4">
        {step === 'radar' ? (
          <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white rounded-[2.5rem] p-4 shadow-2xl border border-slate-100 overflow-hidden">
               ...
               <div className="w-full h-[250px] bg-slate-100 rounded-[2rem] relative overflow-hidden">
                  <div className="absolute inset-0 opacity-30 bg-[url('https://www.google.com/maps/vt/pb=!1m4!1m3!1i14!2i4851!3i9269!2m3!1e0!2sm!3i407105169!3m8!2spt-BR!3sBR!5e1105!12m4!1e68!2m2!1sset!2sRoadmap!4e0!5m1!5f2')] bg-cover"></div>
                  ...
                  {driversOnline.map((driver) => (
                    <div key={driver.id} className="absolute transition-all duration-1000" style={{ left: `${40 + Math.random() * 20}%`, top: `${40 + Math.random() * 20}%` }}>
                        <div className="flex flex-col items-center">
                            ...
                            {/* 5. CORREÇÃO DO RADAR: driver.categoria */}
          ...
          <div className="space-y-4 animate-in fade-in duration-300">
            ...
            <button onClick={handleBuscarRadar} disabled={dist <= 0} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-5 rounded-[2rem] text-lg shadow-xl transition-all uppercase italic tracking-wide">
